		...
		if ($stmt->fetchColumn() == 0) {
			...
			$stmt2->execute(array(':email'=>$email));
				if ($stmt2->fetchColumn() == 0) {
					$sql= 'INSERT INTO utilisateurs (pseudoUtilisateurs,emailUtilisateurs,mdpUtilisateurs,tokenUtilisateurs,etatUtilisateurs,idGroupes) VALUES (:pseudo, :email, :password, :token,0,3)';
					$req=$cnx->prepare($sql);
					$req->execute(array(':pseudo'=>$pseudo,':email'=>$email,':password'=>$password,':token'=>$token))or die(print_r($req->errorInfo()));
